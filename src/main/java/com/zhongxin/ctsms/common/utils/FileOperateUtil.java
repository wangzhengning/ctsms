package com.zhongxin.ctsms.common.utils;
...
public class FileOperateUtil {
     ...
     * @throws Exception
     */
    public static List<PFile> upload(HttpServletRequest request,List<CommonsMultipartFile> multipartFileList,String pathDir) throws Exception {

        List<PFile> files = Lists.newArrayList();

        String uploadDir = request.getSession().getServletContext().getRealPath("/") + pathDir;
        File file = new File(uploadDir);
        ...
        for (MultipartFile multipartFile : multipartFileList) {
            ...
            FileCopyUtils.copy(multipartFile.getInputStream(), zipOutputStream);

            PFile pFile = new PFile();
            pFile.setFileUrl(pathDir + zipName.substring(zipName.lastIndexOf("/")+1));
            pFile.setFileName(multipartFile.getOriginalFilename());
            files.add(pFile);
        }
        return files;
    }
     ...
     * @throws Exception
     */
    public static void download(String url,String contentType, String fileName,HttpServletRequest request, HttpServletResponse response) throws Exception {
        response.setContentType("text/html;charset=UTF-8");
        request.setCharacterEncoding("UTF-8");
        BufferedInputStream bis = null;
        BufferedOutputStream bos = null;
